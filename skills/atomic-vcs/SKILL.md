...
are a pair: **your work is recorded automatically with full AI provenance** (model,
...
How recording happens depends on the environment:

- **Kiro IDE (hooks active):** the AgentStop hook records the turn automatically. You
  never run `atomic add` or `atomic record`.
- **Kiro CLI (manual):** recording is still handled by the orchestrator when you signal
  turn end (`atomic agent hooks kiro agent-stop`) — you don't run raw `atomic add` or
  `atomic record` yourself either.

Either way, **everything in this skill is read-only inspection**, safe to run at any
point in a turn, as often as you like — in both IDE and CLI mode.
...
- **tool executions** — the commands/searches that were run (richest in IDE mode, where
  PreToolUse / PostToolUse hooks capture each tool call)
...
- You don't record (`atomic add`/`record`) yourself in either mode — the IDE hooks do it
  automatically, and in the CLI the orchestrator does it when you signal turn end. These
  commands let you read that recorded provenance back.
...
- For the goals/intents/memory and recording workflow, see the `atomic-vault` skill; for
  building intents, see `intent-builder`.
