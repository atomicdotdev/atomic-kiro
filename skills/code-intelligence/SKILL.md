...
description: Use the Atomic knowledge graph, tree-sitter, and content search to explore code structure and relationships. Use these tools directly instead of grep and find.
---
...
| Find files containing a term | `code "term" -t rs` | ~~find with glob~~ |
...
| Read actual implementation | read the file | Only after `code` or `entities` tells you the line range |
...
5. **Read** the specific lines you need
...
- Chain: code → search → neighbors → entities → read file. Each step narrows your focus.
