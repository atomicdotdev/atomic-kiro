...
This applies in all modes: gather context with `atomic vault query ...` commands before using grep, find, or similar filesystem search tools. Atomic is the primary code-discovery surface; grep/find are fallback tools only when Atomic lacks an index, returns sparse results after `atomic vault query enrich`, or you need untracked files.
...
read file                  → read specific lines you now know about
```
